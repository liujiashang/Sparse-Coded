...
if comm.rank == 0:
    ...
    Matrix_B = scipy.io.loadmat('B.mat')
    Matrix_B = csr_matrix(Matrix_B['B'])
    Matrix_B = Matrix_B.tocoo()

    B_row = np.array(Matrix_B.row, dtype=int)
    B_col = np.array(Matrix_B.col, dtype=int)
    B_data = np.array(Matrix_B.data, dtype=float)
    Bs = []
    Bd = []
    Br = []
    Bc = []
    ...
    print ("Processing time %f" % (received_finished-start_transmission_time))
    print (R_worker)
    ...
    start_receive_from_master = time.time()
    ...
    finish_receive_from_master = time.time()

    # compute result

    result_A = csr_matrix((NR, NCA), dtype=float)
    result_B = csr_matrix((NCA, NC), dtype=float)
    ...
    print("worker %d, %f, %f, %f, %f, %f, %d" % (comm.rank - 1, start_receive_from_master,
                                                 (finish_receive_from_master - start_receive_from_master),
                                                 (finish_compute - finish_receive_from_master),
                                                 (time.time() - finish_compute),
                                                 (time.time() - start_receive_from_master), s[0]))
